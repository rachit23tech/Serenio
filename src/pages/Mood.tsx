...
import type { EmotionState } from "../types/wellness";
...
const ACTIVITIES: Record<MoodLevel, Activity[]> = {
  struggling: [
    ...
    { icon: "🎨✏️", title: "Doodle freely",         desc: "Draw anything. No rules, no talent needed.",                    quote: "Creativity is intelligence having fun.",                          color: "#EA580C", lightBg: "#FFF7ED" },
  ...
  low: [
    ...
    { icon: "🎮🕹️", title: "Play a simple game",     desc: "Puzzle, phone game, anything that occupies your mind.",         quote: "Play is the highest form of research.",                           color: "#7C3AED", lightBg: "#F5F3FF" },
  ...
  okay: [
    ...
    { icon: "🎨🖌️", title: "Try sketching",          desc: "Draw anything for 10 minutes. No judgment, just fun.",         quote: "Every artist was first an amateur.",                              color: "#BE185D", lightBg: "#FDF2F8" },
    ...
    { icon: "💌✍️", title: "Write a letter",         desc: "To your future self or someone you care about.",               quote: "Letters are among the most significant memorials a person can leave.", color: "#DB2777", lightBg: "#FDF2F8" },
  ...
  great: [
    ...
    { icon: "🌍🗺️", title: "Explore somewhere new",  desc: "A new neighbourhood, park or place you've never been.",       quote: "The world is a book. Those who do not travel read only one page.", color: "#059669", lightBg: "#ECFDF5" },
    ...
    { icon: "🎨🖼️", title: "Make something creative", desc: "Channel this great energy into art, writing or music.",      quote: "Creativity is the way I share my soul with the world.",           color: "#EA580C", lightBg: "#FFF7ED" },
...
export default function Mood() {
  ...
  const { scheduleMoodCheckin } = useNotifications();
  const [selected, setSelected] = useState<MoodLevel | null>(null);
  const [activity, setActivity] = useState<Activity | null>(null);
  const [activeTimer, setActiveTimer] = useState<{ title: string; icon: string; duration: number } | null>(null);
  ...
  // Get duration estimate from activity title
  const getActivityDuration = (title: string): number => {
    const durationMap: Record<string, number> = {
      'Box Breathing': 5,
      'Weightless': 10,
      'Make a warm drink': 10,
      'Warm shower': 15,
      'Comfort show': 30,
      'Call someone safe': 10,
      'Dim the lights': 5,
      'Body scan meditation': 10,
      'Write it out': 10,
      'Step outside briefly': 5,
      'Self hug': 2,
      'Doodle freely': 10,
      'Mindful eating': 10,
      'Rest without guilt': 10,
      'Look at something beautiful': 5,
      '10 min slow walk': 10,
      'Journal it out': 10,
      'Fix You': 5,
      'Light stretching': 5,
      'Cook something simple': 20,
      'Watch the sky': 5,
      'Watch something funny': 15,
      'Play a simple game': 15,
      'Tidy one small thing': 10,
      'Pet an animal': 10,
      'Take one photo': 5,
      'Wash your face': 5,
      'Water your plants': 5,
      'Read just one page': 5,
      'Talk to Serenio': 10,
      'Try a new recipe': 45,
      'Read 10 pages': 20,
      'Good Days': 5,
      'Tidy one small space': 10,
      'Try sketching': 10,
      'Go outside': 15,
      'Make a list': 10,
      'Café visit': 30,
      'Watch a documentary': 60,
      'Do a puzzle': 20,
      'Write a letter': 20,
      'Bake something': 30,
      'Podcast walk': 30,
      'Rearrange your space': 20,
      'Check in on someone': 5,
      'Go out for coffee': 20,
      'Call someone': 15,
      'Golden Hour': 5,
      'Quick workout': 20,
      'Capture the day': 10,
      ...
      'Nature walk': 30,
      'Start that book': 30,
      'Try a new restaurant': 60,
      'Write gratitude': 10,
      'Tackle that task': 20,
      'Treat someone': 10,
      'Movie night': 120,
      'Try something physical': 30,
      'Watch the sunset': 10,
      'Dance it out': 10,
      'Plan something exciting': 15,
      'Blinding Lights': 5,
      'Spread the joy': 5,
      'Start that big project': 30,
      'Full workout': 45,
      'Explore somewhere new': 60,
      'Sing out loud': 10,
      'Document this day': 10,
      'Celebrate yourself': 10,
      'Help someone': 20,
      'Learn something new': 30,
      'Wake up early tomorrow': 5,
      'Make something creative': 30,
      'Cook for someone': 60,
    };
      ...
      <main style={{
        ...
        <div style={{ width: "100%", maxWidth: 580 }}>
          ...
          <div style={{ marginBottom: 32, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: t.textMuted, fontFamily: "var(--font-sans)" }}>
          ...
          {selected && (
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <p style={{ fontSize: 13, color: "#66BB6A", fontWeight: 600, fontFamily: "var(--font-sans)" }}>
          ...
          {activity && (
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
                ...
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, margin: "0 0 3px", color: activity.color, textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: "var(--font-sans)" }}>
                    Try this today
                  </p>
                  <p style={{ fontSize: 18, fontWeight: 700, margin: 0, color: dark ? "#F1F5F9" : activity.color, fontFamily: "var(--font-sans)" }}>
                    {activity.title}
                  </p>
                </div>
              </div>

              <p style={{ fontSize: 14, color: t.textPrimary, lineHeight: 1.65, margin: "0 0 16px", fontFamily: "var(--font-sans)" }}>
                {activity.desc}
              </p>
              ...
              <div style={{ padding: "12px 16px", borderRadius: 12, background: dark ? "rgba(255,255,255,0.04)" : activity.color + "11", borderLeft: `3px solid ${activity.color}` }}>
                <p style={{ fontSize: 13, color: dark ? "#CBD5E1" : activity.color, margin: 0, lineHeight: 1.6, fontStyle: "italic", fontFamily: "var(--font-sans)" }}>
                  "{activity.quote}"
                </p>
              </div>
              ...
              <button onClick={handleStartActivity} style={{
                marginTop: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                width: "100%", padding: "12px 18px", borderRadius: 50,
          ...
          {activeTimer && (
            <ActivityTimer
              durationMinutes={activeTimer.duration}
              activityTitle={activeTimer.title}
              activityIcon={activeTimer.icon}
              onComplete={() => {
                setActiveTimer(null);
                setActivity(null);
                setSelected(null);
          ...
          <div style={{ borderRadius: 20, padding: "24px 28px", background: t.card, border: `1px solid ${t.border}`, boxShadow: t.cardShadow }}>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: t.textPrimary, margin: "0 0 20px", fontFamily: "var(--font-sans)" }}>
              7 Day Mood History
            </h2>
              ...
              {days.map((day, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, flex: 1 }}>
                    ...
                    <span style={{ fontSize: 11, color: t.textMuted, fontFamily: "var(--font-sans)" }}>
